...
const handleDownload = (e)=>{
  e.preventDefault();
  window.open(cdnMediaLink, '_blank', 'noopener,noreferrer');
}
    ...
    <div className='text-black font-Poppins font-medium'>
      ...
      {/* main content */}
        <div className="max-w-5xl pt-2 px-3 mx-auto pb-8 p-2">
          ...
          {url ? <img className="inline mx-2 cursor-pointer" width="20" onClick={handleClose} src="/assets/close-icon.png" alt="reel-logo"/>: <img className="inline mr-2 cursor-pointer" onClick={handlePaste} src="/assets/copy-icon.png" alt="reel-logo"/>}
          <button
            type="submit"
        ...
        {/* Reels & download section*/}
        <div className='mt-4 flex flex-col justify-center items-center'>
              {console.log(data.reelLink, "data.reelLink-1")}
              {data.reelLink && url?
                <>
                  <p className='text-sm font-normal mb-2 text-center'>click on downloder button bellow 👇</p>
                  {console.log(`/data/${data.reelLink}`, "data URL 1")}
                  <div className='m-2 flex justify-center'>
                  <video className='rounded-2xl' width="320px" height="auto" controls preload="auto">
                    <source src={`/data/${data.reelLink}`} type="video/mp4"/>
                  </video>
                  </div>
                  <VideoBox videoLink={`/data/${data.reelLink}`}/>
                  {console.log(`/data/${data.reelLink}`, "data URL 2")}
                </> : ""}
              {console.log(data.reelLink, "data.reelLink-2")}
              <div className="max-w-[325px]">
                {cdnMediaLink && url && <button
                    type="submit"
